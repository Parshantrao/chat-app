...
function Message({
    fromCurrentUser,seen=null, senderImage, senderName, lastByUser, content, createdAt, type
}) {

    const date = new Date(createdAt); // Convert timestamp to milliseconds
    ...
    return (
        <div className={cn("flex items-end", {
            ...
            <Avatar className={cn("relative w-8 h-8",{
                ...
                <AvatarImage url={senderImage}/>
                <AvatarFallback>{
                    senderName.substring(0,1)}</AvatarFallback>
            </Avatar>
        </div>
    )
}
